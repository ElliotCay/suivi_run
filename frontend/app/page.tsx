...
import { UploadIcon, Target, Activity, TrendingUp, Sparkles, Award, BarChart3, Calendar, Zap, Brain, Trophy } from 'lucide-react';
...
export default function Home() {
  ...
  const loadProgression = async () => {
    try {
      ...
      const workoutsRes = await axios.get('http://localhost:8000/api/workouts?limit=10');
      const workouts = workoutsRes.data;

      // Get best 5K from records
      const recordsRes = await axios.get('http://localhost:8000/api/records');
        ...
        >
          ...
          >
            <AnimatePresence mode="sync">
              {!conversationStarted ? (
                // Message initial avec quick replies
                <motion.div
                  key="initial-prompt"
                ...
                >
                  <div className="flex justify-start mb-4">
                    <div className="max-w-[85%] md:max-w-[70%] px-5 py-3 rounded-2xl bg-white/20 backdrop-blur-md border border-white/30 text-white">
                  ...
                  {/* Quick replies pour le premier message */}
                  <motion.div
                    initial={{ opacity: 0, y: 10 }}
                    animate={{ opacity: 1, y: 0 }}
                  ...
                  >
                    <motion.button
                      ...
                      onClick={() => {
                        ...
                        setSelectedPath('yes');
                        setSelectedReply("oui, carrément");
                        setTimeout(() => {
                ...
                >
                  {conversation.slice(0, currentMessageIndex + 1).map((message, index) => (
                    <ChatBubble
                      ...
                      speed={message.sender === 'allure' ? 35 : 45}
                    />
                  ))}
                    ...
                    >
                      ...
                      <div className="flex justify-start">
                        <button
                          ...
                          className="max-w-[85%] md:max-w-[70%] px-4 py-2 rounded-2xl bg-white/15 backdrop-blur-md border border-white/25 text-white/90 hover:bg-white/20 transition-all text-sm text-left"
                        >
                          {expandedSection === 'features' ? '−' : '+'} que fait allure exactement ?
                        </button>
                      </div>
                      <AnimatePresence>
                      ...
                      <div className="flex justify-start">
                        <button
                          ...
                          className="max-w-[85%] md:max-w-[70%] px-4 py-2 rounded-2xl bg-white/15 backdrop-blur-md border border-white/25 text-white/90 hover:bg-white/20 transition-all text-sm text-left"
                        >
                          {expandedSection === 'pricing' ? '−' : '+'} ça coûte combien ?
                        </button>
                      </div>
                      <AnimatePresence>
                  ...
                  {showFinalContent && (
                    ...
                    >
                      <Link href="/import">
                        <Button size="lg" className="text-lg px-10 py-6 shadow-2xl bg-white text-black hover:bg-white/90 hover:scale-105 transition-all font-bold rounded-full">
          ...
          {/* Quick Reply Buttons - like iMessage */}
      ...
      {hasData && progression && (
        <section className="py-16 px-4">
          <div className="container mx-auto max-w-5xl">
            ...
            >
              <Card className="border-white/20 bg-black/60 backdrop-blur-md overflow-hidden">
